...
#include "envoy/extensions/attempt/concurrency_budget/v3/concurrency_budget_config.pb.h"
...
public:
  ConcurrencyBudgetConfigTest() : cb_stats_(clusterCircuitBreakersStats(store_)) {
    Upstream::AttemptAdmissionControllerFactory* factory =
        Registry::FactoryRegistry<Upstream::AttemptAdmissionControllerFactory>::getFactory(
            "envoy.attempt.concurrency_budget");
...
protected:
  void createAdmissionController() {
    admission_controller_ = factory_->createAdmissionController(
        config_, ProtobufMessage::getStrictValidationVisitor(), runtime_, runtime_prefix_,
        cb_stats_, *store_.rootScope());
  }

  void configureConcurrencyBudget(uint32_t min_limit, double budget_percent) {
    auto* budget = config_.mutable_retry_budget();
    budget->mutable_min_retry_concurrency()->set_value(min_limit);
    budget->mutable_budget_percent()->set_value(budget_percent);
  }

  std::unique_ptr<Upstream::AttemptAdmissionControllerFactory> factory_;
  NiceMock<Runtime::MockLoader> runtime_;
  TestScopedRuntime scoped_runtime_;
  Stats::IsolatedStoreImpl store_;
  Upstream::ClusterCircuitBreakersStats cb_stats_;
  std::string runtime_prefix_{"test_prefix."};
  envoy::extensions::attempt::concurrency_budget::v3::ConcurrencyBudgetConfig config_;
  NiceMock<StreamInfo::MockStreamInfo> request_stream_info_;
  Upstream::AttemptAdmissionControllerSharedPtr admission_controller_;
  Upstream::AttemptStreamAdmissionControllerPtr retry_stream_admission_controller_;
};
...
TEST_F(ConcurrencyBudgetConfigTest, FactoryDefault) {
  ...
  EXPECT_TRUE(retry_stream_admission_controller_->isInitialAttemptAdmitted());
  retry_stream_admission_controller_->onTryStarted(1);
  ASSERT_TRUE(retry_stream_admission_controller_->isAttemptAdmitted(1, 2, false));
  retry_stream_admission_controller_->onTryStarted(2);
  ASSERT_TRUE(retry_stream_admission_controller_->isAttemptAdmitted(2, 3, false));
  retry_stream_admission_controller_->onTryStarted(3);
  ASSERT_TRUE(retry_stream_admission_controller_->isAttemptAdmitted(3, 4, false));
  retry_stream_admission_controller_->onTryStarted(4);
  ASSERT_FALSE(retry_stream_admission_controller_->isAttemptAdmitted(4, 5, false));
  ...
  std::vector<Upstream::AttemptStreamAdmissionControllerPtr> extra_streams;
  extra_streams.reserve(16);
  for (int i = 0; i < 16; i++) {
    extra_streams.push_back(
        admission_controller_->createStreamAdmissionController(request_stream_info_));
    EXPECT_TRUE(extra_streams.back()->isInitialAttemptAdmitted());
  ...
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 0);
  ASSERT_TRUE(extra_streams.front()->isAttemptAdmitted(1, 2, false));
  extra_streams.front()->onTryStarted(2);
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
  ASSERT_FALSE(extra_streams.front()->isAttemptAdmitted(2, 3, false));
}
...
TEST_F(ConcurrencyBudgetConfigTest, FactoryRuntimeOverrides) {
  ...
  // by default, 3 retries are allowed
  EXPECT_TRUE(stream1->isInitialAttemptAdmitted());
  stream1->onTryStarted(1);
  ASSERT_TRUE(stream1->isAttemptAdmitted(1, 2, false));
  stream1->onTryStarted(2);
  ASSERT_TRUE(stream1->isAttemptAdmitted(2, 3, false));
  stream1->onTryStarted(3);
  ASSERT_TRUE(stream1->isAttemptAdmitted(3, 4, false));
  stream1->onTryStarted(4);
  ASSERT_FALSE(stream1->isAttemptAdmitted(4, 5, false));
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
  // 4 active tries and 3 active retries
  ...
  EXPECT_TRUE(stream2->isInitialAttemptAdmitted());
  stream2->onTryStarted(1);
  ASSERT_TRUE(stream2->isAttemptAdmitted(1, 2, false));
  stream2->onTryStarted(2);
  ASSERT_FALSE(stream2->isAttemptAdmitted(2, 3, false));
  ...
  EXPECT_TRUE(stream3->isInitialAttemptAdmitted());
  stream3->onTryStarted(1);                            // 7T, 4R
  ASSERT_TRUE(stream3->isAttemptAdmitted(1, 2, true)); // 7T, 5R
  stream3->onTryStarted(2);
  ASSERT_FALSE(stream3->isAttemptAdmitted(2, 3, false)); // 6 / 8 = 75% > 72%
}

TEST_F(ConcurrencyBudgetConfigTest, FactoryConfigured) {
  configureConcurrencyBudget(1, 75.0);
  createAdmissionController();
  ...
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 0);
  stream1->onTryStarted(1);
  ASSERT_TRUE(stream1->isAttemptAdmitted(1, 2, true)); // 1T, 1R
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
  stream1->onTryStarted(2);
  ASSERT_FALSE(stream1->isAttemptAdmitted(2, 3, false)); // 1T, 1R
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
  ...
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 0);
  ASSERT_TRUE(stream3->isAttemptAdmitted(1, 2, false)); // 4T, 2R
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 1);   // 4 * 75% = 3 => 3 retries allowed
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 0);
  stream3->onTryStarted(2);
  ASSERT_TRUE(stream3->isAttemptAdmitted(2, 3, false)); // 5T, 3R
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);   // 5 * 75% = 3.75 => 3 retries allowed
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
  stream3->onTryStarted(3);
  ASSERT_FALSE(stream3->isAttemptAdmitted(3, 4, false)); // denied: still 5T, 3R
  ASSERT_EQ(cb_stats_.remaining_retries_.value(), 0);
  ASSERT_EQ(cb_stats_.rq_retry_open_.value(), 1);
...
TEST_F(ConcurrencyBudgetConfigTest, AbortOnStreamDestruction) {
  configureConcurrencyBudget(1, 0);
  createAdmissionController();
  ...
  ASSERT_FALSE(stream1->isAttemptAdmitted(2, 3, false)); // 3T, 2R => max = 1 < 2 => denied
  stream1.reset();

  stream2->onTryStarted(1);                             // 1T, 0R
  ASSERT_TRUE(stream2->isAttemptAdmitted(1, 2, false)); // 2T, 1R
}

TEST_F(ConcurrencyBudgetConfigTest, AbortPreviousOnRetry) {
  configureConcurrencyBudget(1, 60.0);
  createAdmissionController();
  ...
  stream1->onTryStarted(1);                             // 1T, 0R
  ASSERT_TRUE(stream1->isAttemptAdmitted(1, 2, false)); // 2T, 1R
  stream1->onTryStarted(2);
  ASSERT_TRUE(
      stream1->isAttemptAdmitted(2, 3, true)); // 2T, 1R still due to abort of attempt 2 (retry)
  ...
  ASSERT_FALSE(stream2->isAttemptAdmitted(1, 2, true));
}

TEST_F(ConcurrencyBudgetConfigTest, EmptyConfig) {
  ProtobufTypes::MessagePtr config = factory_->createEmptyConfigProto();
  EXPECT_TRUE(
      dynamic_cast<envoy::extensions::attempt::concurrency_budget::v3::ConcurrencyBudgetConfig*>(
          config.get()));
}
